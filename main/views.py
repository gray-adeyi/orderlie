...
class RegisterStudent(FormView):
    ...
    def form_valid(self, form):
        ...
        slug = models.Class.objects.get(
            name=form.cleaned_data.get("student_class")
        ).slug
...
# TODO: BAD CODE, convert to simple and maintainable code.
def download_class_data(request, slug):
    ...
    if request.query_params.get("file-format") == "xlsx":
        wb = Workbook()
        sheet = wb.active
        ...
        headers = [
            ...
            "Middlename",
            "Matric No",
        ]  # Headers if names are required to be discrete.
        headers_name_merged = [
            "S/N",
            "Names",
            "Matric No",
        ...
        if request.query_params.get("names") == "merged":
            ...
            for index, row in enumerate(
                sheet.iter_rows(
                    min_row=1, max_col=3, max_row=len(members_name_merged) + 1
                )
            ):
                if index == 0:
            ...
            for row in range(2, len(members_name_merged) + 2):
                for idx, col in enumerate(headers_name_merged):
                    sheet.cell(
                        row=row, column=idx + 1, value=members_name_merged[row - 2][idx]
                    )

        else:  # alternate logic if names are required to be discrete.
            for index, row in enumerate(
                sheet.iter_rows(min_row=1, max_col=5, max_row=len(members) + 1)
            ):
                if index == 0:
                    for idx, cell in enumerate(row):
        ...
        response[
            "Content-Disposition"
        ] = f"inline; filename={student_class.name}-bio-data.xlsx"
        response["Content-Transfer-Encoding"] = "binary"
        response.write(temp_file.read())
        temp_file.close()
        return response
    elif request.query_params.get("file-format") == "docx":
        doc = Document()
        doc.add_heading(f"{student_class.name} Bio-Data")
        p = doc.add_paragraph(f"Session: {student_class.get_session_display()}")
        ...
        if request.query_params.get("names") == "merged":
            ...
            for idx, member in enumerate(student_class.students.all()):
                ...
                row_cells[
                    1
                ].text = f"{member.last_name.upper()} {member.first_name.upper()} {member.middle_name.upper()}"
        ...
        response[
            "Content-Disposition"
        ] = f"inline; filename={student_class.name}-bio-data.docx"
        response["Content-Transfer-Encoding"] = "binary"
        response.write(temp_file.read())
        temp_file.close()
        return response
    else:
        return Response({"detail": "Format not supported"})
